<<<<<<< HEAD
=======
# Creative collaborations: Aesthetic, Cultural,and Institutional Proximity and the Formation of Joint Production in Music {background-color="#2D6A7A"}
...
- **Integración**: un único modelo combina homofilia estética, cultural-geográfica e institucional y por actividad — nivel medio y equilibrio del par — y estructura de red.
...
<<<<<<< HEAD
- **Formación, no repetición**: la formación exige que no hubiera ningún lazo previo entre ambos; las repeticiones se estudian aparte (el resultado agregado suma 1.811 eventos frente a 1.064 de formación).
- **Pares invitado–invitado**: no cuentan como vínculo, pero tampoco como negativos.
=======
...
## ¿Optimo interior?
<<<<<<< HEAD
La especificación cuadrática para la proximidad estética impone un máximo por construcción. Pero, ¿alcanza la propensión a colaborar un máximo interior de similitud estética, o crece y se satura?
:::
...
<<<<<<< HEAD
![](figures/fig_coef_dotwhisker_base.svg){width="84%" fig-align="center" fig-alt="Gráfico de puntos e intervalos del 95% para coeficientes seleccionados del logit de formación, modelo Completo: bloques de red, estética, actividad y atributos"}

::: {style="font-size:0.8em; color:#6c757d;"}
Coeficientes seleccionados del modelo Completo (los comentados en la exposición); IC 95% con EE diádicos Aronow–Samii. Las escalas no son comparables entre variables.
:::
...
|---|---|---|
<<<<<<< HEAD
| **Inferencia** | *bootstrap* de nodos | el *bootstrap* aumenta los EE (ratio mediano 1,60): los términos estéticos, la geodésica, el grado, la escena y el tipo se mantienen; el historial común de sello deja de distinguirse de cero |
...
<<<<<<< HEAD
![](figures/fig_coef_delta_firth_cloglog.svg){width="84%" fig-align="center" fig-alt="Distancias entre estimaciones puntuales, Firth menos logit y cloglog menos logit, para los coeficientes seleccionados del modelo base"}

::: {style="font-size:0.8em; color:#6c757d;"}
Mismos coeficientes que en la lectura gráfica del modelo base; solo estimaciones puntuales (sin EE recalculados). Cloglog está en otra escala de enlace: la distancia no se interpreta como sesgo.
=======
...
<<<<<<< HEAD
![](figures/fig_coef_delta_clogit.svg){width="78%" fig-align="center" fig-alt="Distancias entre las estimaciones del logit condicional principal por año y el logit base para los coeficientes seleccionados no de red"}

::: {style="font-size:0.8em; color:#6c757d;"}
Mismos coeficientes que en el modelo base, sin el bloque de red (fijo dentro del estrato). El estimando cambia: lectura de dirección y estabilidad, no de magnitud.
=======
...
<<<<<<< HEAD
- **Asociaciones condicionales, no causalidad**: estimaciones condicionales a la red retardada, no efectos causales.
=======
...
<<<<<<< HEAD
- **Principal — adecuación de red**: las simulaciones desde el modelo estimado reproducen el volumen de nuevos vínculos, pero no su concentración en determinados nodos (dependencia residual no capturada); refinar esa estrategia de simulación para evaluar la incertidumbre del modelo y la adecuación de la independencia condicional entre díadas.
- **Sobre los resultados**: poblar la cola de alta similitud con horizontes más largos y densificar los vocabularios de etiquetas; la infraestructura versionada (listas, etiquetas fechadas, panel diádico) sostiene estas líneas dentro de CREAMARKT.
=======
- **Metodológica**: Almquist y Butts(2014) sugieren evaluar adecuación del modelo *logit* para la red. Simulaciones dinámicas del modelo estimado reproducen el volumen de nuevos vínculos pero muestran capacidad limitada para capturar la concentración en determinados nodos (dependencia residual no capturada). Refinar esa estrategia de simulación para evaluar la incertidumbre del modelo y la adecuación de la independencia condicional entre díadas.
