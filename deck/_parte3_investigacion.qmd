...
:::{.notes}
Presento la propuesta de investigación *Creative Collaborations*, que estudia la formación de colaboraciones entre artistas en la música grabada. La pregunta que organiza toda la exposición es fácil de formular: en la producción en las industrias culturales ¿que factores están asociados con la propensión a colaborar?
...
::: {.notes}
La aportación central del trabajo versa sobre la afinidad estética su medición y su impacto asimétrico. Desde un punto de vista más amplio, la contribución se articula en torno a cuatro elementos. 
...
Representamos la naturaleza relacional de los datos mediante un grafo que refleja los vínculos de colaboración entre artistas. Esta red evoluciona entre 2015 y 2024, y en la modelización se persigue capturar si la posición  en la red predice la propensión a colaborar, una vez consideradas otras medidas de proximidad.
...
- **Variantes de sensibilidad**: ventana de cinco años y sin salida (reportadas como sensibilidad).
...
::: {.notes}
Para la estimación se recurre a dos herramientas. El logit es el modelo explicativo: modeliza la probabilidad de formación condicionada a la red del año anterior, a las características  del par e incluye efectos fijos de año.  Permite hacer inferencia sobre una forma que se especifica
de forma explícita. 
...
::: {.notes}
Comenzamos analizando los resultados de la estimación base para el modelo de formación. La tabla muestra los coeficientes estimados para las diferentes especificaciones. Huyo del exceso de detalle y paso a analizar un subconjunto de los coeficientes estimados.
...
::: {.notes}
En el ejercicio predictivo se estiman los dos modelos (logit y xgbm) con datos de 2015 a 2023 y se predice una sola vez en 2024, de modo que el modelo evaluado nunca ha visto el año de evaluación. En 2024 hay 64 eventos de formación de 490.000 (modelo logit) o 526.000 (para xgbm) puntuadas según el modelo, una tasa base de 0,12 a 0,13 por mil. La diferencia viene porque el logit se entrena para el conjunto de casos completos y XGBoost para todos los pares, gracias al tratamiento nativo de los valores ausentes.
...
::: {.notes}
Dada la baja frecuencia del evento analizado, se presenta
...
::: {.notes}
El grafico visualiza el movimiento en la estimación puntual en Firth. Los resultado muestran que no se produce un cambio de signo 
...
## Firth: distancia entre estimaciones {.compact}

&nbsp;

![](figures/fig_coef_delta_firth.svg){width="70%" fig-align="center" fig-alt="Distancia entre estimaciones puntuales, Firth menos logit, para los coeficientes seleccionados del modelo base, coloreada por bloque de variables; ningún coeficiente cambia de signo"}
...
::: {.notes}
El grafico visualiza el movimiento en la estimación puntual en Firth. Los resultado muestran que no se produce un cambio de signo
en ninguna de las variables principales (tampoco en las no mostradas) y que la magnitud del desplazamiento de los coeficientes es pequeña en términos relativos: el mayor desplazamiento de toda la tabla es 0,176, en el índice de Jaccard  que equivale a aproximadamente 0,23 veces el error estándar.
...
## Firth: el patrón apenas se mueve {.compact}
...
## Logit condicional: dos excepciones en un patrón estable {.compact}
...
**La formación de colaboraciones se concentran por afinidad, acceso relacional y oportunidad reciente.**
...
::::: {.column width="23.5%"}
::: {style="text-align:center;"}
Las simulaciones del modelo reproducen el volumen de nuevos vínculos, no su concentración: dependencia residual entre díadas.
:::
:::::
::::::
...
::::: {.column width="23.5%"}
::: {style="text-align:center;"}
Refinar la simulación y evaluar la verosimilitud de la independencia condicional (Almquist y Butts, 2014).
:::
:::::
::::::
